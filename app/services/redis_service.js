...
module.exports = {
  getConnection: async () => {
    if (myPool === undefined) {
      ...
      const opts = {
        max: Number(process.env.REDIS_MAX_CON_POOL)
          ? Number(process.env.REDIS_MAX_CON_POOL)
          : 10, // maximum size of the pool
        min: Number(process.env.REDIS_MIN_CON_POOL)
          ? Number(process.env.REDIS_MIN_CON_POOL)
          : 2, // minimum size of the pool
      }
